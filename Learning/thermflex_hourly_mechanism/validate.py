...
def build_grouped_holdout_split(
    *,
    ...
    if group_column not in truth_df.columns:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped split column missing from truth dataframe: "
            f"{group_column}"
        )
    if truth_df.empty:
        raise ValueError("[thermflex_hourly_mechanism] cannot split an empty truth dataframe.")
    groups = truth_df[group_column].astype(str)
    unique_groups = sorted(groups.unique().tolist())
    if len(unique_groups) < 2:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped holdout requires at least two distinct groups in "
    ...
    if strategy == "group_shuffle":
        ...
        raise ValueError(
            "[thermflex_hourly_mechanism] unsupported grouped split strategy: "
            f"{split_strategy}"
        )
    return SplitResult(
...
def _build_group_stratified_indices(
    *,
    ...
    The ThermFlex tau slices can contain very different seasonal mechanisms.
    This helper keeps group leakage impossible while still preventing a holdout
    that accidentally contains only the low-variance winter subset.
    """

    if stratify_column is None:
        raise ValueError(
            "[thermflex_hourly_mechanism] group_stratified_shuffle requires an explicit stratify_column."
        )
    stratify_name = str(stratify_column)
    if stratify_name not in truth_df.columns:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped stratified split column missing from truth dataframe: "
            f"{stratify_name}"
        )
    ...
    if not too_small.empty:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped stratified split requires at least two groups per stratum; "
    ...
    if n_test < n_strata or (n_groups - n_test) < n_strata:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped stratified split has too few train/test groups "
...
def _single_row_per_group(
    *,
    ...
    if not mixed.empty:
        raise ValueError(
            "[thermflex_hourly_mechanism] grouped stratified split requires one stratum per group; "
