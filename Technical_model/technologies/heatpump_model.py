...
def simulate_heatpump_heating_system(params, profiles):
    ...
    # building path already constructed the member-level reference heat demand.
    # This keeps the HP electricity calculation aligned with the same active
    # building-demand source that feeds DH and thermflex.
    if "space_heat_demand_override_kwh" in profiles:
        heating_load_on_days = np.asarray(profiles["space_heat_demand_override_kwh"], dtype=float)
    else:
        from Technical_model.consumption.heating_anc_cooling_consumption.heating_and_cooling import (
            get_heating_load_on_days,
        )

        building_params = params.get('building', {})
        heating_load_on_days = get_heating_load_on_days(profiles=profiles, building_params=building_params)
...
def simulate_heatpump_cooling_system(params, profiles):
    # Cooling still uses the legacy reduced-order helper until the teacher-derived
    # runtime cooling path is introduced explicitly.
    from Technical_model.consumption.heating_anc_cooling_consumption.heating_and_cooling import (
        get_cooling_load_on_days,
    )

    building_params = params.get('building', {})
    cooling_load_on_days = get_cooling_load_on_days(profiles=profiles, building_params=building_params)
    T_outdoor_array = profiles['T_outdoor']
...
if __name__ == "__main__":
    # NOTE: This test block requires member_ids. For standalone testing, use prepare_profiles_adapter instead.
    # This block is kept for backward compatibility but may not work without proper member configuration.
    location = "Vienna"
    try:
        # Try to load profiles (will fail if member_ids required)
        profiles = data.load_profiles(location)
    except ValueError as e:
        print(f"Error: {e}")
        print("This test block requires member_ids. Use prepare_profiles_adapter() instead.")
        raise
    params = data.get_parameters(location)
    params['location'] = location
    params['building'] = data.technologies_global['building']
