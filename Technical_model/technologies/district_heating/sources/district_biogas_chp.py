...
def _require_config_value(config: Any, attr: str) -> float:
    if config is None or not hasattr(config, attr):
        raise ValueError(f"[district_biogas_chp] Missing required config attribute '{attr}'.")
    value = getattr(config, attr)
    if value is None:
        raise ValueError(f"[district_biogas_chp] Config attribute '{attr}' must not be None.")
    return float(value)
...
def _is_scheduled_downtime(timestamp: Any, config: Any) -> bool:
    ...
    if not 1 <= start_day <= 366:
        raise ValueError("[district_biogas_chp] scheduled_downtime_start_day_of_year must be within [1, 366].")
    if not 0 <= days <= 366:
        raise ValueError("[district_biogas_chp] scheduled_downtime_days_per_year must be within [0, 366].")
...
def dispatch_district_biogas_chp(
    ...
    if requested_thermal_kwh < 0.0:
        raise ValueError("[district_biogas_chp] requested_thermal_kwh must be >= 0.")
    if installed_kw_el < 0.0:
        raise ValueError("[district_biogas_chp] installed_kw_el must be >= 0.")
    if dt_h <= 0.0:
        raise ValueError("[district_biogas_chp] dt_h must be > 0.")
    ...
    fuel_lhv_kwh_per_nm3 = _require_config_value(config, "fuel_lhv_kwh_per_nm3")
    if not 0.0 < eta_el <= 1.0:
        raise ValueError("[district_biogas_chp] eta_el must be within (0, 1].")
    if not 0.0 <= eta_th <= 1.0:
        raise ValueError("[district_biogas_chp] eta_th must be within [0, 1].")
    if not 0.0 <= min_partload <= 1.0:
        raise ValueError("[district_biogas_chp] min_partload must satisfy 0 <= min_partload <= 1.")
    if fuel_lhv_kwh_per_nm3 <= 0.0:
        raise ValueError("[district_biogas_chp] fuel_lhv_kwh_per_nm3 must be > 0.")
