...
def _require_config_value(config: Any, attr: str) -> float:
    if config is None or not hasattr(config, attr):
        raise ValueError(f"[district_gas_boiler] Missing required config attribute '{attr}'.")
    value = getattr(config, attr)
    if value is None:
        raise ValueError(f"[district_gas_boiler] Config attribute '{attr}' must not be None.")
    return float(value)
...
def _is_scheduled_downtime(timestamp: Any, config: Any) -> bool:
    ...
    if not 1 <= start_day <= 366:
        raise ValueError("[district_gas_boiler] scheduled_downtime_start_day_of_year must be within [1, 366].")
    if not 0 <= days <= 366:
        raise ValueError("[district_gas_boiler] scheduled_downtime_days_per_year must be within [0, 366].")
...
def dispatch_district_gas_boiler(
    ...
    if requested_thermal_kwh < 0.0:
        raise ValueError("[district_gas_boiler] requested_thermal_kwh must be >= 0.")
    if installed_kw_th < 0.0:
        raise ValueError("[district_gas_boiler] installed_kw_th must be >= 0.")
    if dt_h <= 0.0:
        raise ValueError("[district_gas_boiler] dt_h must be > 0.")
    ...
    if not 0.0 <= min_partload <= max_partload <= 1.0:
        raise ValueError(
            "[district_gas_boiler] Partload settings must satisfy 0 <= min_partload <= max_partload <= 1."
        )
    if not 0.0 < eta_th <= 1.0:
        raise ValueError("[district_gas_boiler] eta_th must be within (0, 1].")
    if fuel_lhv_kwh_per_m3 <= 0.0:
        raise ValueError("[district_gas_boiler] fuel_lhv_kwh_per_m3 must be > 0.")
