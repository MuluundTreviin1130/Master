...
# Alles Ã¼ber das zentrale Settings-File
from Settings import get_settings
# gleiche Logik fÃ¼r runs_root wie im Orchestrator
...
# ---------------------------------------------------------------------------
# Konfiguration â€“ NUR Dinge, die nicht im Settings-File stehen
# ---------------------------------------------------------------------------
...
# â†’ HIER ggf. anpassen: z.B. "pareto_points.csv" oder "pareto_front.csv"
PARETO_CSV_NAME = "pareto_points.csv"

# Spaltennamen im Pareto-CSV â€“ bitte mit deiner CSV prÃ¼fen
NPC_COL = "npc_eur"
PEF_COL = "pef_pt"
...
def _load_latest_pareto(runs_root: Path, location: str,
    ...
    """
    Holt den letzten Run Ã¼ber _LATEST_RUN.json und lÃ¤dt das Pareto-CSV.
    """
...
def _add_autarky(df: pd.DataFrame,
    ...
    Wenn das CSV jÃ¤hrliche Importe enthÃ¤lt, ist lifetime_load_kwh
    einfach die Jahreslast â†’ gleiche Formel.
    """
...
def main() -> None:
    ...
    # 3) Jahres- und Lifetime-Last berechnen (fÃ¼r Autarkie)
    ...
    print(f"  Î”NPC   (abs) : {npc_red_abs:.2f} EUR  (negativ = V2H gÃ¼nstiger)")
    print(f"  Î”NPC   (rel) : {npc_red_rel * 100:.2f} %")

    print(f"  Î”PEF   (abs) : {pef_red_abs:.2f} pt   (negativ = V2H Ã¶kologischer)")
    print(f"  Î”PEF   (rel) : {pef_red_rel * 100:.2f} %")

    print(f"  Î”Autarkie    : {aut_inc_abs:.4f}  (positiv = V2H autarker)")
